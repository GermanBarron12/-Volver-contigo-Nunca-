...
test('No se permiten numeros negativos', () {
  expect(() => calcularAguaAtrapada([5, -3, 2]), throwsA(isA<noNegativos>()));
});

test('Un solo negativo debe lanzar error', () {
  expect(() => calcularAguaAtrapada([1, 2, -1, 4]), throwsA(isA<noNegativos>()));
});

test('Negativo al inicio debe lanzar error', () {
  expect(() => calcularAguaAtrapada([-5, 3, 2]), throwsA(isA<noNegativos>()));
});

test('Negativo al final debe lanzar error', () {
  expect(() => calcularAguaAtrapada([3, 2, -1]), throwsA(isA<noNegativos>()));
});
}